                    ...
                    <?php foreach ($social_media_links as $social): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button onclick="editSocial(<?= $social['id'] ?>)" class="text-blue-600 hover:text-blue-900 mr-3">Edit</button>
...
function editSocial(id) {
    ...
    // Find the social media data from the PHP-loaded data
    const socials = <?= json_encode($social_media_links) ?>;
    ...
    if (social) {
                ...
                document.getElementById('link_url').value = social.link_url;
                document.getElementById('sort_order').value = social.sort_order || '';
        document.getElementById('is_active').checked = social.is_active == 1;
        ...
        // Find and highlight the matching icon
        for (let [iconName, svgPath] of Object.entries(iconMap)) {
            if (svgPath === social.icon_svg) {
                const matchingOption = document.querySelector(`[data-icon="${iconName}"]`);
                if (matchingOption) {
                    matchingOption.classList.remove('border-gray-200');
                    matchingOption.classList.add('border-black', 'bg-gray-50');
                }
                break;
            }
        }
        ...
        showAlert('Error loading social media: Social media not found', 'error');
            }
}
...
document.getElementById('saveSocialBtn').addEventListener('click', function() {
    ...
    fetch('setting.php', {
        ...
        body: JSON.stringify({
            action: currentAction === 'add' ? 'add' : 'edit',
            table: 'social_media',
            id: currentId,
            data: data
        })
    ...
    .then(data => {
        if (data.success) {
            showAlert(`Social media ${currentAction === 'add' ? 'added' : 'updated'}!`, 'success');
